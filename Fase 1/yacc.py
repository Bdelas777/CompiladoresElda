...
# Regla de error que se activará si falta el TOKEN_MAIN
...
def p_type(p):
    '''type : TOKEN_INT
    | TOKEN_FLOAT'''
    # Aquí validamos explícitamente que solo int y float son tipos válidos
    if p[1] not in ['int', 'float']:
        ...
        p[0] = 'int'  # Tipo por defecto en caso de error
    else:
        p[0] = p[1]
def p_body(p):
...
def p_funcs(p):
    ...
    # Process parameters from tipo with mejor manejo de errores
    params = p[5] if p[5] else []
    if isinstance(params, list):
        for param in params:
            if isinstance(param, tuple) and len(param) >= 3 and param[0] == 'param':
                # Validar que el tipo sea válido antes de agregarlo
                param_id = param[1]
                param_type = param[2]
                
                # Si el tipo no es válido, lo marcaremos como error pero seguiremos procesando
